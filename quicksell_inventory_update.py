...
def quicksell():
    url = 'https://affinityfootwear.catalog.to/s/affinity-footwear-co/footwear/1u2'
...
def json_data(resp_data):
    # products = resp_data.find('script').text.strip()
    products = resp_data.find("meta", {"name":"amalgam"})['content'].strip()
    #print(products)
    
    # trim_data = products.replace('<script type="text/javascript">', "").replace('window.amalgamObject = ', '').replace(
        # '</script>', '')
    #f= open("guru99.txt","w+")
...
def saveimages(product):
    if not os.path.exists('productsImages/' + str(product['product_name'] + " " + product['pid']) + '/'):
        imgnam = 0
        for product_picture in product['product_pics']:
            picture = product_picture
            print(picture, imgnam)
            resp = get_image(picture)
            if not os.path.exists('productsImages/' + str(product['product_name'] + " " + product['pid']) + '/'):
                os.makedirs('productsImages/' + str(product['product_name'] + " " + product['pid']) + '/')
            imagefile = str(imgnam) + '.jpg'
            output = open('productsImages/' + str(product['product_name'] + " " + product['pid']) + '/' + imagefile,
                          'wb')
